#!/bin/bash
# ========================================================
# INSTALADOR LIVRE - MENU COMPLETO PARA VPS (SEM CHAVE)
# PERSONALIZADO PARA: ROBERT.SSH.PLUS
# ========================================================

# Definição de Cores Padrão (Idênticas às da imagem)
VERMELHO='\033[1;31m'
VERDE='\033[1;32m'
AMARELO='\033[1;33m'
AZUL='\033[1;34m'
...
# Cores de Fundo para o Topo
BG_VERMELHO='\033[41;1;37m'

# Captura de Dados do Sistema em Tempo Real
...
ONLINES=$(ps aux | grep -E "sshd|dropbear" | grep -v grep | wc -l) # Estimativa simples de onlines

clear
while true; do
    HORA_ATUAL=$(date +%H:%M:%S)
    
    # Desenho do Menu - Alinhamento Fixo para Termius
    echo -e "${AZUL}┌────────────────────────────────────────────────────────┐${SEM_COR}"
    echo -e "${AZUL}│${BG_VERMELHO}                      ROBERT.SSH.PLUS                    ${SEM_COR}${AZUL}│${SEM_COR}"
    echo -e "${AZUL}├────────────────────────────────────────────────────────┤${SEM_COR}"
    ...
    # Colunas de Opções (01 a 23) separadas e alinhadas perfeitamente
    printf "${AZUL}│${AMARELO} [%02d] ${AZUL}• ${SEM_COR}%-20s ${AMARELO}[%02d] ${AZUL}• ${SEM_COR}%-17s${AZUL}│\n" 1 "CRIAR USUARIO" 13 "SPEEDTEST"
    printf "${AZUL}│${AMARELO} [%02d] ${AZUL}• ${SEM_COR}%-20s ${AMARELO Graves}[%02d] ${AZUL}• ${SEM_COR}%-17s${AZUL}│\n" 2 "CRIAR TESTE" 14 "OTIMIZAR"
    ...
    case $opcao in
        1|01)
            # Se você tiver os submódulos criados pelo instalador.sh, chama aqui:
            bash /etc/sshplus/criarusuario
            ;;
        12)
            bash /etc/sshplus/conexao
            ;;
        0|00)
            clear
            exit 0
            ;;
        *)
            echo -e "\n${VERMELHO}Opção Inválida!${SEM_COR}"
            sleep 1
            ;;
    esac
done
